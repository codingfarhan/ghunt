...
class PeoplePaHttp(GAPI):
    ...
    async def batch_get_info_by_emails(self, as_client: httpx.AsyncClient, emails: list) -> list:
        """
        Batch fetch information for multiple email addresses.

        Args:
            as_client: The HTTP client instance.
            emails: A list of email addresses.
            params_template: The request parameter template.

        Returns:
            A list of tuples with (success: bool, Person object).
        """
        endpoint_name = "batch_get_info_by_emails"
        verb = "POST"
        base_url = "/v2/people:batchLookup"
        data_type = "json"
            ...
            "requests": [
                {
                    ...
                    "match_type": "EXACT",
                    "request_mask": {
                        "include_field": {"paths": ["person.metadata", "person.name"]}
                    }
                } for email in emails
            ]
        }
        ...
        self._load_endpoint(endpoint_name)
        req = await self._query(as_client, verb, endpoint_name, base_url, None, batch_payload, data_type)

        results = []
        response_data = json.loads(req.text)
        for response in response_data.get("responses", []):
            person = Person()
            if response.get("status", {}).get("code", 0) == 200:
                await person._scrape(as_client, response.get("person", {}))
                results.append((True, person))
            else:
                results.append((False, person))
    ...
    async def batch_get_info_by_gaia_ids(self, as_client: httpx.AsyncClient, gaia_ids: list) -> list:
        """
        Batch fetch information for multiple Gaia IDs.

        Args:
            as_client: The HTTP client instance.
            gaia_ids: A list of Gaia IDs.
            params_template: The request parameter template.

        Returns:
            A list of tuples with (success: bool, Person object).
        """
        endpoint_name = "batch_get_info_by_gaia_ids"
        verb = "POST"
        base_url = "/v2/people:batchGet"
        data_type = "json"
